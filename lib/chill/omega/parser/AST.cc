...
AST_constraints::AST_constraints(std::set<ast::Exp *> *f, Rel_Op r, AST_constraints *o) {
  others = o;
  rel_op = r;
  first = f;
}

AST_constraints::AST_constraints(std::set<ast::Exp *> *f, Rel_Op r, std::set<ast::Exp *> *s) {
  others = new AST_constraints(s);
  rel_op = r;
  first = f;
}

AST_constraints::AST_constraints(std::set<ast::Exp *> *f){
  others = 0;
  first = f;
}

AST_constraints::~AST_constraints() {
  for (std::set<ast::Exp *>::iterator i = first->begin(); i != first->end(); i++)
    delete *i;
  delete first;
  delete others;
}
...
void AST_constraints::print() {
  for (std::set<ast::Exp *>::iterator i = first->begin(); ;) {
...
ast::Exp::Exp(coef_t c) : coefs() {
  constantTerm = c;
}

ast::Exp::Exp(Variable_Ref *v) : coefs() {
  assert(v != 0);
  constantTerm = 0;
  coefs[v] = 1;
}
...
ast::Exp *multiply (coef_t c, ast::Exp *x) {
  x->constantTerm *= c;
  for (std::map<Variable_Ref *, omega::coef_t>::iterator i = x->coefs.begin(); i != x->coefs.end(); i++)
    (*i).second *= c;
  return x;
...
void AST_constraints::install(Formula *F) {
  ...
  for (std::set<ast::Exp *>::iterator i = first->begin(); i != first->end(); i++)
    for (std::set<ast::Exp *>::iterator j = others->first->begin(); j != others->first->end(); j++)
      switch (rel_op) {
...
void install_neq(F_And *F, ast::Exp *e1, ast::Exp *e2) {
  F_Or *or_ = F->add_or();
...
void install_eq(F_And *F, ast::Exp *e1, ast::Exp *e2) {
...
void install_geq(F_And *F, ast::Exp *e1, ast::Exp *e2) {
...
void install_gt(F_And *F, ast::Exp *e1, ast::Exp *e2) {
  GEQ_Handle c = F->add_GEQ();
  c.update_const(-1);
  c.update_const(e1->constantTerm);
  if (e2) c.update_const(-e2->constantTerm);
...
void tupleDescriptor::extend(ast::Exp *e) {
  Variable_Ref *r  = relationDecl->extend();
  size++;
  vars.push_back(r);
  assert(size == vars.size());
  ast::Exp *eq = subtract(e, new ast::Exp(r));
  eq_constraints.insert(eq); 
}

void tupleDescriptor::extend(char *s, ast::Exp *e) {
  Variable_Ref *r  = relationDecl->extend(s);
  size++;
  vars.push_back(r);
  assert(size == vars.size());
...
void tupleDescriptor::extend(ast::Exp *lb,ast::Exp *ub) {
  Variable_Ref *r  = relationDecl->extend();
  size++;
  vars.push_back(r);
  assert(size == vars.size());
  ast::Exp *lb_exp = subtract(new ast::Exp(r), lb);
  geq_constraints.insert(lb_exp); 
  ast::Exp *ub_exp = subtract(ub, new ast::Exp(r));
  geq_constraints.insert(ub_exp); 
}
void tupleDescriptor::extend(ast::Exp *lb,ast::Exp *ub, coef_t stride) {
  Variable_Ref *r  = relationDecl->extend();
  size++;
  vars.push_back(r);
  ast::Exp *lb_exp = subtract(new ast::Exp(r), new ast::Exp(*lb));
  geq_constraints.insert(lb_exp); 
  ast::Exp *ub_exp = subtract(ub, new ast::Exp(r));
  geq_constraints.insert(ub_exp); 
  strideConstraint *s = new strideConstraint;
  s->e = subtract(lb, new ast::Exp(r));
  s->step = stride;
