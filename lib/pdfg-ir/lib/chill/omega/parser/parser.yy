...
%destructor {
  for (std::set<ast::Exp *>::iterator i = $$->begin(); i != $$->end(); i++)
    delete *i;
  delete $$;
 } expList;
%destructor {
...
%right NOT APPROX DOMAIN_ RANGE HULL PROJECT_AWAY_SYMBOLS PROJECT_ON_SYMBOLS DIFFERENCE DIFFERENCE_TO_RELATION INVERSE COMPLEMENT FARKAS SAMPLE SYM_SAMPLE MAKE_UPPER_BOUND MAKE_LOWER_BOUND p7
%left p8
%nonassoc GIVEN
%left p9
%left '(' p10
...
builtRelation :  tupleDeclaration GOES_TO {currentTuple = Output_Tuple;} 
                 ...
                 r->setup_names();
                 for (std::set<ast::Exp *>::iterator i = $1->eq_constraints.begin(); i != $1->eq_constraints.end(); i++)
                   install_eq(f, *i, 0);
                 for (std::set<ast::Exp *>::iterator i = $1->geq_constraints.begin(); i != $1->geq_constraints.end(); i++)
                   install_geq(f, *i, 0);
                 for (std::set<strideConstraint *>::iterator i = $1->stride_constraints.begin(); i != $1->stride_constraints.end(); i++)
                   install_stride(f, *i);
                 for (std::set<ast::Exp *>::iterator i = $4->eq_constraints.begin(); i != $4->eq_constraints.end(); i++)
                   install_eq(f, *i, 0);
                 for (std::set<ast::Exp *>::iterator i = $4->geq_constraints.begin(); i != $4->geq_constraints.end(); i++)
                   install_geq(f, *i, 0);
...
tupleVar : VAR %prec p10 {
           ...
           if (!ds)
             ...
             if (ds != globalDecls)
               currentTupleDescriptor->extend($1, new ast::Exp(v));
             else
...
expList : exp ',' expList {$$ = $3; $$->insert($1);}
        | exp {$$ = new std::set<ast::Exp *>(); $$->insert($1);}
;
